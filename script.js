...
// Adaptado a navegación inferior simple
// =============================================
...
const EMOJI_MAP = {
  "ajo":"🧄","cebolla":"🧅","tomate":"🍅","champi":"🍄","champiñon":"🍄","carne":"🥩","pollo":"🍗",
  "pescado":"🐟","queso":"🧀","pasta":"🍝","harina":"🌾","arroz":"🍚","default":"📦"
};
...
function cargarElaboraciones() {
  ...
  Object.keys(RECETAS).forEach(elab => {
    html += `
      <button class="btn-elab" onclick="seleccionarElab('${elab}', this)">
        ${elab}
      </button>`;
  });
...
function abrirFotoIngrediente(i, tipo) {
  ...
  if (tipo === 'cam') {
    document.getElementById('inputIngredienteFotoCam').click();
  } else {
    document.getElementById('inputIngredienteFotoGal').click();
  }
}
...
async function guardarSesion() {
  ...
  if (ingredientes.length === 0) {
    ...
    return alert("Debes seleccionar al menos un ingrediente.");
  }
...
async function cargarProductos() {
  ...
  lista.innerHTML = '<div style="color:var(--muted);text-align:center;padding:40px;">Cargando productos...</div>';
...
function renderListaProductos() {
  ...
  if (!productosLibreria.length) {
    lista.innerHTML = '<div style="color:var(--muted);text-align:center;padding:40px;">Aún no hay productos registrados.</div>';
    return;
  }
  lista.innerHTML = productosLibreria.map((p, i) => `
...
async function guardarProducto() {
  ...
  try {
    ...
    showSuccess("Producto Guardado", nombre + " añadido a la biblioteca", "📚");
